...
const config = {
  libraryName: '@browninglogic/ng-modal',
  unscopedLibraryName: 'ng-modal',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /ng-modal/ --deploy-url /ng-modal/`, { cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, { cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run @browninglogic/ng-modal-demo:server`, { cwd: `${config.demoDir}` }))
